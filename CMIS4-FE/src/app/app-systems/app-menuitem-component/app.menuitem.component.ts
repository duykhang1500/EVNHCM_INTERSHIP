...
import { MenuService } from '../app-menu-component/app.menu.service';
...
@Component({
    ...
    template: `
        <ng-container>
            ...
            [routerLink]="item.routerLink" [attr.target]="item.target" [attr.tabindex]="0" [ngClass]="item.class" (mouseenter)="onMouseEnter()" pRipple
               [pTooltip]="item.label" [tooltipDisabled]="active || !(root && app.isSlim() && !app.isMobile())">
                ...
                <i class="pi pi-fw pi-angle-down layout-submenu-toggler" *ngIf="root && item.items"></i>
            </a>
            ...
            <ul *ngIf="((item.items && root) || (item.items && active)) && item.visible !== false" [@children]="root ? 'visible' : active ? 'visibleAnimated' : 'hiddenAnimated'">
                <ng-template ngFor let-child let-i="index" [ngForOf]="item.items">
                    <li [item]="child" [index]="i" [parentKey]="key"></li>
                </ng-template>
            </ul>
        </ng-container>
    `,
